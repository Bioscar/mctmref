...
where $\mathbf{a}_r$ and $\mathbf{b}_r$ are Bernstein basis vectors of order 6, $\boldsymbol{\nu}_{r}$ are response-specific coefficients, and $\boldsymbol{\beta}_r$ capture the effect of gender and age on the marginal distributions (for $r = 1, 2$).
